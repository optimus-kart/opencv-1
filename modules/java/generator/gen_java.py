...
T_JAVA_START_INHERITED = read_contents(os.path.join(
    SCRIPT_DIR, 'templates/java_class_inherited.prolog'))
T_JAVA_START_ORPHAN = read_contents(os.path.join(
    SCRIPT_DIR, 'templates/java_class.prolog'))
T_JAVA_START_MODULE = read_contents(os.path.join(
    SCRIPT_DIR, 'templates/java_module.prolog'))
T_CPP_MODULE = Template(read_contents(os.path.join(
    SCRIPT_DIR, 'templates/cpp_module.template')))


class GeneralInfo():
    def __init__(self, type, decl, namespaces):
        self.symbol_id, self.parent_id, self.namespace, self.classpath, self.classname, self.name = self.parseName(
            decl[0], namespaces)
    ...
    def parseName(self, name, namespaces):
        ...
        '''
        name = name[name.find(
            " ")+1:].strip()  # remove struct/class/const prefix
    ...
    def fullParentNameJAVA(self):
        result = ".".join([f for f in [self.namespace] +
                           self.classpath.split(".") if len(f) > 0])
        return result
...
class ClassInfo(GeneralInfo):
    def __init__(self, decl, namespaces=[]):  # [ 'class/struct cname', ': base', [modlist] ]
        ...
        if decl[1]:
            ...
            base_info = ClassInfo(
                ('class {}'.format(base_class), '', [], [], None, None), [self.namespace])
            base_type_name = base_info.name
            if not base_type_name in type_dict:
                base_type_name = re.sub(
                    r"^.*:", "", decl[1].split(",")[0]).strip().replace(self.jname, "")
...
class JavaWrapperGenerator(object):
    ...
    def clear(self):
        self.namespaces = ["cv"]
        classinfo_Mat = ClassInfo(
            ['class cv.Mat', '', [], []], self.namespaces)
    ...
    def add_class(self, decl):
        ...
        if name in missing_consts:
            if 'private' in missing_consts[name]:
                for (n, val) in missing_consts[name]['private']:
                    classinfo.private_consts.append(
                        ConstInfo([n, val], addedManually=True))
            if 'public' in missing_consts[name]:
                for (n, val) in missing_consts[name]['public']:
                    classinfo.consts.append(
                        ConstInfo([n, val], addedManually=True))
        ...
        logging.info('ok: class %s, name: %s, base: %s',
                     classinfo, name, classinfo.base)

    def add_const(self, decl, enumType=None):  # [ "const cname", val, [], [] ]
        constinfo = ConstInfo(
            decl, namespaces=self.namespaces, enumType=enumType)
    ...
    def add_enum(self, decl):  # [ "enum cname", "", [], [] ]
        ...
        if enumType.endswith("<unnamed>"):
            ...
            type_dict[ctype] = {"cast_from": "int", "cast_to": get_cname(
                enumType), "j_type": "int", "jn_type": "int", "jni_type": "jint", "suffix": "I"}
        const_decls = decl[3]
    ...
    def add_func(self, decl):
        ...
        class_symbol_id = classname if self.isWrapped(classname) else fi.classpath.replace(
            '.', '_')  # ('.'.join([fi.namespace, fi.classpath])[3:])
    ...
    def gen(self, srcfiles, module, output_path, output_jni_path, output_java_path, common_headers):
        ...
        for hdr in srcfiles:
            ...
            for decl in decls:
                logging.info("\n--- Incoming ---\n%s",
                             pformat(decl[:5], 4))  # without docstring
                name = decl[0]
        ...
        for ci in sorted(self.classes.values(), key=lambda x: x.symbol_id):
            ...
            self.save("%s/%s/%s.java" %
                      (output_java_path, module, ci.jname), classJavaCode)
            moduleCppCode.write(ci.generateCppCode())
            ci.cleanupCodeStreams()
        cpp_file = os.path.abspath(os.path.join(
            output_jni_path, module + ".inl.hpp"))
        self.cpp_files.append(cpp_file)
        self.save(cpp_file, T_CPP_MODULE.substitute(m=module, M=module.upper(
        ), code=moduleCppCode.getvalue(), includes="\n".join(includes)))
    ...
    def gen_func(self, ci, fi, prop_name=''):
        ...
        if prop_name:
            ...
            for a in fi.args:
                ...
                if a.defval:
                    s += " = "+a.defval
                decl_args.append(s)
            c_decl = "%s %s %s(%s)" % (fi.static, fi.ctype,
                                       fi.cname, ", ".join(decl_args))
        ...
        if fi.ctype not in type_dict:  # unsupported ret type
            ...
            j_code.write(" "*4 + msg)
            logging.warning("SKIP:" + c_decl.strip() +
        ...
        while True:
            ...
            for a in args:
                ...
                if "v_type" in type_dict[a.ctype]:  # pass as vector
                    # pass as Mat or vector_Mat
                    if type_dict[a.ctype]["v_type"] in ("Mat", "vector_Mat"):
                        jn_args.append(
                            ArgInfo(["__int64", "%s_mat.nativeObj" % a.name, "", [], ""]))
                        jni_args.append(
                            ArgInfo(["__int64", "%s_mat_nativeObj" % a.name, "", [], ""]))
                        c_prologue.append(type_dict[a.ctype]["jni_var"] % {
                                          "n": a.name} + ";")
                        c_prologue.append(
                            "Mat& %(n)s_mat = *((Mat*)%(n)s_mat_nativeObj)" % {"n": a.name} + ";")
                        if "I" in a.out or not a.out:
                            if type_dict[a.ctype]["v_type"] == "vector_Mat":
                                j_prologue.append(
                                    "List<Mat> %(n)s_tmplm = new ArrayList<Mat>((%(n)s != null) ? %(n)s.size() : 0);" % {"n": a.name})
                                j_prologue.append("Mat %(n)s_mat = Converters.%(t)s_to_Mat(%(n)s, %(n)s_tmplm);" % {
                                                  "n": a.name, "t": a.ctype})
                            else:
                                if not type_dict[a.ctype]["j_type"].startswith("MatOf"):
                                    j_prologue.append("Mat %(n)s_mat = Converters.%(t)s_to_Mat(%(n)s);" % {
                                                      "n": a.name, "t": a.ctype})
                                else:
                                    j_prologue.append(
                                        "Mat %s_mat = %s;" % (a.name, a.name))
                            c_prologue.append("Mat_to_%(t)s( %(n)s_mat, %(n)s );" % {
                                              "n": a.name, "t": a.ctype})
                        else:
                            if not type_dict[a.ctype]["j_type"].startswith("MatOf"):
                                j_prologue.append(
                        ...
                        if "O" in a.out:
                            if not type_dict[a.ctype]["j_type"].startswith("MatOf"):
                                j_epilogue.append("Converters.Mat_to_%(t)s(%(n)s_mat, %(n)s);" % {
                                                  "t": a.ctype, "n": a.name})
                                j_epilogue.append("%s_mat.release();" % a.name)
                            c_epilogue.append("%(t)s_to_Mat( %(n)s, %(n)s_mat );" % {
                                              "n": a.name, "t": a.ctype})
                    else:  # pass as list
                        jn_args.append(ArgInfo([a.ctype, a.name, "", [], ""]))
                        jni_args.append(
                            ArgInfo([a.ctype, "%s_list" % a.name, "", [], ""]))
                        c_prologue.append(type_dict[a.ctype]["jni_var"] % {
                                          "n": a.name} + ";")
                        if "I" in a.out or not a.out:
                            c_prologue.append("%(n)s = List_to_%(t)s(env, %(n)s_list);" % {
                                              "n": a.name, "t": a.ctype})
                        if "O" in a.out:
                            c_epilogue.append("Copy_%s_to_List(env,%s,%s_list);" % (
                                a.ctype, a.name, a.name))
                else:
                    fields = type_dict[a.ctype].get(
                        "jn_args", ((a.ctype, ""),))
                    ...
                    if "I" in a.out or not a.out or self.isWrapped(a.ctype):
                        for f in fields:
                            ...
                            jni_args.append(
                                ArgInfo([f[0], a.name + normalize_field_name(f[1]), "", [], ""]))
                    # out arg, pass as double[]
                    if "O" in a.out and not self.isWrapped(a.ctype):
                        jn_args.append(
                            ArgInfo(["double[]", "%s_out" % a.name, "", [], ""]))
                        jni_args.append(
                            ArgInfo(["double[]", "%s_out" % a.name, "", [], ""]))
                        j_prologue.append(
                        ...
                        if type_dict[a.ctype]["j_type"] in ('bool', 'int', 'long', 'float', 'double'):
                            j_epilogue.append('if(%(n)s!=null) %(n)s[0] = (%(t)s)%(n)s_out[0];' % {
                                              'n': a.name, 't': type_dict[a.ctype]["j_type"]})
                        else:
                            set_vals = []
                            i = 0
                            for f in fields:
                                set_vals.append("%(n)s%(f)s = %(t)s%(n)s_out[%(i)i]" %
                                                {"n": a.name, "t": (
                                                    "("+type_dict[f[0]]["j_type"]+")", "")[f[0] == "double"], "f": f[1], "i": i}
                                                )
                                i += 1
                            j_epilogue.append(
                                "if("+a.name+"!=null){ " + "; ".join(set_vals) + "; } ")
            ...
            jn_code.write(Template(
                ...
                args=", ".join(["%s %s" % (
                    type_dict[a.ctype]["jn_type"], normalize_field_name(a.name)) for a in jn_args])
            ))
            ...
            if fi.docstring:
                ...
                for index, line in enumerate(lines):
                    ...
                    if p0 != -1:
                        ...
                        if "</code>" in line:
                            inCode = False
                        # @result is valid in Doxygen, but invalid in Javadoc
                        ...
                        if index == len(lines) - 1:
                            for arg in j_args:
                                ...
                                if not name in javadocParams:
                                    toWrite.append(
                                        " * @param " + name + " automatically generated")
            ...
            if fi.ctype.endswith('*'):
                ret_type = ret_type[:-1]
            ret_val = type_dict[ret_type]["j_type"] + \
                " retVal = " if j_epilogue else "return "
            tail = ""
            ...
            if "v_type" in type_dict[ret_type]:
                ...
                if type_dict[ret_type]["v_type"] in ("Mat", "vector_Mat"):
                    ...
                    if j_type.startswith('MatOf'):
                        ...
                        j_prologue.append(
                            j_type + ' retVal = new Array' + j_type+'();')
                        j_epilogue.append(
                            'Converters.Mat_to_' + ret_type + '(retValMat, retVal);')
                        ret = "return retVal;"
            elif ret_type.startswith("Ptr_"):
                constructor = type_dict[ret_type]["j_type"] + ".__fromPtr__("
                if j_epilogue:
                    ret_val = type_dict[fi.ctype]["j_type"] + \
                        " retVal = " + constructor
                else:
...
"""
                ...
                " " if type_dict[fi.ctype]["j_type"] else "",
                j_name=fi.jname,
                j_args=", ".join(j_args),
            ...
            if fi.ctype == "void":
                ...
                if self.isSmartClass(ci):
                    ret = "return (jlong)(new Ptr<%(ctype)s>(_retval_));" % {
                        'ctype': fi.fullClassCPP()}
                else:
                ...
                ret = "return (jlong) new %s(_retval_);" % self.fullTypeNameCPP(
                    fi.ctype)
            elif fi.ctype.startswith('Ptr_'):
                c_prologue.append("typedef Ptr<%s> %s;" % (
                    self.fullTypeNameCPP(fi.ctype[4:]), fi.ctype))
                ret = "return (jlong)(new %(ctype)s(_retval_));" % {
                    'ctype': fi.ctype}
            ...
            retval = self.fullTypeNameCPP(
                fi.ctype) + " _retval_ = " if ret else "return "
            if fi.ctype == "void":
                retval = ""
            elif fi.ctype == "String":
                retval = "cv::" + \
                    self.fullTypeNameCPP(fi.ctype) + " _retval_ = "
            elif fi.ctype == "string":
                retval = "std::string _retval_ = "
            elif "v_type" in type_dict[fi.ctype]:  # vector is returned
                retval = type_dict[fi.ctype]['jni_var'] % {
                    "n": '_ret_val_vector_'} + " = "
                if type_dict[fi.ctype]["v_type"] in ("Mat", "vector_Mat"):
                    c_epilogue.append("Mat* _retval_ = new Mat();")
                    c_epilogue.append(
                        fi.ctype+"_to_Mat(_ret_val_vector_, *_retval_);")
                else:
                    if ret:
                        c_epilogue.append(
                            "jobject _retval_ = " + fi.ctype + "_to_List(env, _ret_val_vector_);")
                    else:
                        c_epilogue.append(
                            "return " + fi.ctype + "_to_List(env, _ret_val_vector_);")
            if fi.classname:
                if not fi.ctype:  # c-tor
                    if self.isSmartClass(ci):
                        retval = self.smartWrap(
                            ci, fi.fullClassCPP()) + " _retval_ = "
                    ...
                    cvname = ("me->" if not self.isSmartClass(ci)
                              else "(*me)->") + name
                    c_prologue.append(
            ...
            for a in args:
                ...
                if not a.ctype:  # hidden
                    jni_name = a.defval
                cvargs.append(type_dict[a.ctype].get(
                    "jni_name", jni_name) % {"n": a.name})
...
""").substitute(
                ...
                args=", ".join(
                    ["%s %s" % (type_dict[a.ctype].get("jni_type"), a.name) for a in jni_args]),
                argst=", ".join([type_dict[a.ctype].get("jni_type")
                                 for a in jni_args]),
                prologue="\n        " +
                ...
                retval=retval,
                namespace=('using namespace ' + ci.namespace.replace('.', '::') +
                           ';') if ci.namespace and ci.namespace != 'cv' else ''
            ))
    ...
    private static final int
        ...
        if ci.consts:
            ...
            grouped_consts = {enumType: [
                c for c in ci.consts if c.enumType == enumType] for enumType in enumTypes}
    ...
    public static final int
        ...
        for pi in ci.props:
            ...
            if pi.rw:
                ...
                fi = FuncInfo([setter_name, "void", [], [
                              [pi.ctype, pi.name, "", [], ""]]], self.namespaces)
                self.gen_func(ci, fi, pi.name)
...
""" % {"module": module.replace('_', '_1'), "cls": self.smartWrap(ci, ci.fullNameCPP()), "j_cls": ci.jname.replace('_', '_1')}
    ...
    def finalize(self, output_jni_path):
        ...
        self.save(list_file, '\n'.join(
            ['#include "%s"' % f for f in self.cpp_files]))
...
def copy_java_files(java_files_dir, java_base_path, default_package_path='org/opencv/'):
    ...
    for root, dirnames, filenames in os.walk(java_files_dir):
        java_files += [os.path.join(root, filename)
                       for filename in filenames if re_filter.match(filename)]
    ...
    for java_file in java_files:
        ...
        target_fname = (m.group(1) + '.' + m.group(2)
                        ) if m else os.path.basename(java_file)
        ...
        dest = os.path.join(java_base_path, os.path.join(
            package_path, target_fname))
...
def sanitize_java_documentation_string(doc, type):
    ...
    doc = re.sub(re.compile('\\\\f\\$(.*?)\\\\f\\$', re.DOTALL),
                 '\\(' + r'\1' + '\\)', doc)
    doc = re.sub(re.compile(
        '\\\\f\\[(.*?)\\\\f\\]', re.DOTALL), '\\(' + r'\1' + '\\)', doc)
    doc = re.sub(re.compile(
        '\\\\f\\{(.*?)\\\\f\\}', re.DOTALL), '\\(' + r'\1' + '\\)', doc)
    ...
    doc = re.sub(re.compile(
        '\\*\\*([^\\*]+?)\\*\\*', re.DOTALL), '<b>' + r'\1' + '</b>', doc)

    lines = doc.splitlines()

    lines = list(map(lambda x: x[x.find('*'):].strip()
                     if x.lstrip().startswith("*") else x, lines))
    ...
    for index, line in enumerate(lines[:]):
        if line.strip().startswith("-"):
            ...
            if not listInd or i > listInd[-1]:
                ...
                lines.insert(index + indexDiff, "  "*len(listInd) + "<li>")
                indexDiff += 1
            lines[index + indexDiff] = lines[index + indexDiff][0:i] + \
                lines[index + indexDiff][i + 1:]
        else:
    ...
    lines = list(map(lambda x: "* " +
                     x[1:].strip() if x.startswith("*") and x != "*" else x, lines))
    lines = list(map(lambda x: x if x.startswith(
        "*") else "* " + x if x and x != "*" else "*", lines))

    lines = list(map(lambda x: x
                     .replace("@note", "<b>Note:</b>"), lines))

    lines = list(map(lambda x: re.sub(
        '@b ([\\w:]+?)\\b', '<b>' + r'\1' + '</b>', x), lines))
    lines = list(map(lambda x: re.sub(
        '@c ([\\w:]+?)\\b', '<tt>' + r'\1' + '</tt>', x), lines))
    lines = list(map(lambda x: re.sub(
        '`(.*?)`', "{@code " + r'\1' + '}', x), lines))
    lines = list(map(lambda x: re.sub(
        '@p ([\\w:]+?)\\b', '{@code ' + r'\1' + '}', x), lines))
...
if __name__ == "__main__":
    # initialize logger
    logging.basicConfig(filename='gen_java.log', format=None,
    ...
    arg_parser = argparse.ArgumentParser(
        description='OpenCV Java Wrapper Generator')
    arg_parser.add_argument(
        '-p', '--parser', required=True, help='OpenCV header parser')
    arg_parser.add_argument(
        '-c', '--config', required=True, help='OpenCV modules config')
    ...
    FILES_REMAP = {os.path.realpath(os.path.join(
        ROOT_DIR, f['src'])): f['target'] for f in config['files_remap']}
    logging.info("\nRemapped configured files (%d):\n%s",
                 len(FILES_REMAP), pformat(FILES_REMAP))
    ...
    for e in config['modules']:
        (module, module_location) = (e['name'],
                                     os.path.join(ROOT_DIR, e['location']))
        ...
        if os.path.exists(srcfiles_fname):
            with open(srcfiles_fname) as f:
                srcfiles = [os.path.join(module_location, str(
                    l).strip()) for l in f.readlines() if str(l).strip()]
        else:
            re_bad = re.compile(
                r'(private|.inl.hpp$|_inl.hpp$|.details.hpp$|_winrt.hpp$|/cuda/|/legacy/)')
            # .h files before .hpp
            h_files = []
            hpp_files = []
            for root, dirnames, filenames in os.walk(os.path.join(module_location, 'include')):
                h_files += [os.path.join(root, filename)
                            for filename in fnmatch.filter(filenames, '*.h')]
                hpp_files += [os.path.join(root, filename)
                              for filename in fnmatch.filter(filenames, '*.hpp')]
            srcfiles = h_files + hpp_files
            srcfiles = [f for f in srcfiles if not re_bad.search(
                f.replace('\\', '/'))]
        ...
        if os.path.exists(common_headers_fname):
            with open(common_headers_fname) as f:
                common_headers = [os.path.join(module_location, str(
                    l).strip()) for l in f.readlines() if str(l).strip()]
        logging.info("\nCommon headers (%d):\n%s", len(
            common_headers), pformat(common_headers))
        ...
        if os.path.exists(gendict_fname):
            ...
            if 'module_j_code' in gen_type_dict:
                module_j_code = read_contents(checkFileRemap(
                    os.path.join(misc_location, gen_type_dict['module_j_code'])))
            if 'module_jn_code' in gen_type_dict:
                module_jn_code = read_contents(checkFileRemap(
                    os.path.join(misc_location, gen_type_dict['module_jn_code'])))
        ...
        if os.path.exists(java_test_files_dir):
            copy_java_files(java_test_files_dir,
                            java_test_base_path, 'org/opencv/test/' + module)

        if len(srcfiles) > 0:
            generator.gen(srcfiles, module, dstdir, jni_path,
                          java_path, common_headers)
        else:
